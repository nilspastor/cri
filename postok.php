...
switch($action)
 {
  ...
  case "nouveautopic": // NOUVEAU TOPIC
  ...
  if (empty($message) || empty($titre))
  ...
  else
   {
    ...
    $query=$db->prepare('UPDATE forum_apat SET apat_post = apat_post + 1 WHERE apat_id = :id');
    $query->bindValue(':id', $id, PDO::PARAM_INT);   
    $query->execute();
    $query->CloseCursor();
	...
	case "repondre": // REPONDRE AU TOPIC
    $message = $_POST['message'];
	...
	case "repondremp": // REPONDRE A UN MP
	...
	case "supprimer": // SUPPRIMER UN MP
	...
	if (!verif_auth($data1['auth_modo']) && $data1['post_createur'] != $id)
	...
	else
	 {
	  ...
	  echo '<p>Le message a été édité avec succès !<br /><br />
	  Cliquez <a href="./index.php">ici</a> pour revenir à l\'index du forum !<br />
	...
	if (!verif_auth($data['auth_modo']) && $poster != $id)
	...
	else
	 {
	  ...
	  if ($data_post['topic_first_post']==$post) // Cas 1er message
	   {
		...
		echo '<p>La suppression du premier post entraînera également celle du topic.<br />
		<a href="./postok.php?action=delete_topic&amp;t='.$topic.'">Supprimer le topic</a> - <a href="./voirtopic.php?t='.$topic.'">Revenir en arrière</a></p>';
	
	    $query->CloseCursor();
		
	   }
	
	  elseif ($data_post['topic_last_post']==$post) // Cas dernier message
	   {
		...
		$query=$db->prepare('UPDATE forum_apat SET apat_post = apat_post - 1
		WHERE apat_id = :id');
		$query->bindValue(':id',$poster,PDO::PARAM_INT);
		$query->execute();
		$query->CloseCursor();
		
		echo '<p>Le message a été correctement effacé !<br />
		Cliquez <a href="./voirtopic.php?t='.$topic.'">ici</a> pour revenir au topic.<br />
        Cliquez <a href="./index.php">là</a> pour retourner à l\'index du forum.</p>';
	  ...
	  else // Cas classique
	   {
		...
		$query=$db->prepare('UPDATE forum_forum SET forum_post = forum_post - 1 
		WHERE forum_id = :forum');
		$query->bindValue(':forum',$forum,PDO::PARAM_INT);
		$query->execute();
		$query->CloseCursor();
	...
	case "delete_topic": // SUPPRIMER UN TOPIC
    ...
    if (!verif_auth($data['auth_modo']))
    ...
    else
     {
	  ...
	  while($data = $query->fetch())
	   {
	    ...
	    $query=$db->prepare('UPDATE forum_apat
		SET apat_post = apat_post - :mess
		WHERE apat_id = :id');
	  ...
	  echo '<p>Le topic a été correctement effacé !<br />
	...
	case "lock": // VERROUILLER UN TOPIC
    ...
    if (!verif_auth($data['auth_modo']))
    ...
    else
     {
	  ...
	  echo '<p>Ce topic est désormais verrouillé.<br />
	  Cliquez <a href="./voirtopic.php?t='.$topic.'">ici</a> pour revenir au topic<br />
      Cliquez <a href="./index.php">là</a> pour retourner à l\'index du forum</p>';
	...
	case "unlock": // DEVERROUILLER UN TOPIC
	...
	if (!verif_auth($data['auth_modo']))
	...
	else
	 {
	  ...
	  echo '<p>Le topic a été deverrouillé avec succès !<br />
	  Cliquez <a href="./voirtopic.php?t='.$topic.'">ici</a> pour revenir au topic<br />
      Cliquez <a href="./index.php">là</a> pour retourner à l\'index du forum</p>';
	...
	case "deplacer": // DEPLACER UN TOPIC
    ...
    if (!verif_auth($data['auth_modo']))
    ...
    else
     {
      ...
      echo'<p>Le topic a été déplacé avec succès !<br />
      Cliquez <a href="./voirtopic.php?t='.$topic.'">ici</a> pour revenir au topic<br />
	...
	// Laissez un blanc, a modifier dans la suite du cours (partie admin)
